...
The plugin logs full detail (including stack traces for schema parsing and serialisation failures) at `DEBUG`, but only returns a short message to the Pact framework. Set `LOG_LEVEL=DEBUG` in the environment that launches the plugin process and check `log/plugin.log` in the plugin directory (or standard output) before digging further. See xref:logging.adoc[].

If a response says `Multiple errors detected and logged, please check logs`, the individual causes are only in the log file, not in the error returned to your test framework.
...
|The embedded schema lookup described in xref:schema_compatibility.adoc[] failed: the interaction's `schemaKey` is missing, or no matching entry exists in the Pact file's `pactConfiguration`. This points at a corrupted or manually assembled Pact file rather than a schema authoring mistake.
